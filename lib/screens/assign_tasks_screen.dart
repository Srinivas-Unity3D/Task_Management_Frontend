...
class _AssignTasksScreenState extends State<AssignTasksScreen> {
  ...
  void _handleNewNotification(dynamic data) {
    ...
    if (mounted) {
      if (data['type'] == 'task_created' || data['type'] == 'task_updated') {
        ...
        print(
            '🔄 AssignTasksScreen - Task assignment received, refreshing assignments...');
        Future.delayed(const Duration(milliseconds: 500), () {
          _fetchAssignments();
        });
      }
  ...
  Future<void> _loadUserAndAssignments() async {
    ...
    if (_currentUserId != null && _currentUsername != null) {
      print(
          '✅ AssignTasksScreen - User data loaded: ID=$_currentUserId, Username=$_currentUsername, Role=$_currentRole');
      // Connect socket
      print(
          '🔄 AssignTasksScreen - Connecting socket for user: $_currentUsername');
      ...
      print(
          '⚠️ AssignTasksScreen - User data missing, not fetching assignments');
      if (mounted) {
        setState(() {
          _isLoading = false;
        });
        ...
        try {
          ...
          if (assignments != null && _currentUsername != null) {
            if (!_isAdmin) {
              ...
              print(
                  '🔄 AssignTasksScreen - Filtered ${assignments.length} tasks assigned by $_currentUsername');
            } else {
              print(
                  '🔄 AssignTasksScreen - Showing all ${assignments.length} tasks for admin user');
            }
          }
        } catch (e) {
  ...
  Widget _buildTaskAssignmentItem(TaskAssignment assignment) {
    ...
    // Determine the status emoji based on currentTask
    ...
    return InkWell(
      ...
      child: Container(
        ...
        padding: const EdgeInsets.all(12), // Updated to 12 from 16
        decoration: BoxDecoration(
          color: AppColors.cardBackground,
          borderRadius: BorderRadius.circular(12),
          boxShadow: [
        ...
        child: Row(
          ...
          children: [
            ...
            const SizedBox(width: 8), // Updated to 8 from 12
            // Task Details
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  ...
                  Row(
                    children: [
                      const Text(
                        'Assigned To: ', // Updated to "Assigned By"
                        style: TextStyle(
                          fontSize: 12,
                          color: AppColors.textGrey,
                        ),
                  ...
                  Row(
                    children: [
                      ...
                      // Status badge
                      Text(
                        _getStatusEmoji(assignment.currentTask),
                    ...
                    children: [
                      ...
                      Flexible(
                        ...
                        child: ConstrainedBox(
                          ...
                          child: Text(
                            'Created: ${_formatDate(assignment.createdDate)} | ${_formatDateOnly(assignment.createdDate)}',
                            style: const TextStyle(
                              color: AppColors.textGrey,
                              fontSize: 10,
                            ),
                      ...
                      const SizedBox(width: 8), // Updated to 8 from 16
                      const Text(
                        '📅 ',
                        style: TextStyle(fontSize: 12),
                      ),
                      Flexible(
                        ...
                        child: Text(
                          'Due: ${_formatDateOnly(assignment.dueDate)}',
                          style: const TextStyle(
                            color: AppColors.textGrey,
                            fontSize: 10,
                          ),
  ...
  //     final period = istDate.hour >= 12 ? 'PM' : 'AM';
  //     final time =
  //         '${hour.toString().padLeft(2, '0')}:${istDate.minute.toString().padLeft(2, '0')} $period';
  //     // print('Time:---> ${assignment.createdDate}');
  //     // print(istDate.day);
  //     return time;
  //     // }
  ...
  //     final istDate = date.add(const Duration(hours: 5, minutes: 30));
  //     final fullDate =
  //         '${istDate.year}-${istDate.month.toString().padLeft(2, '0')}-${istDate.day.toString().padLeft(2, '0')}';
  ...
  //                     ),
  //                     Text(
  //                       'Created: ${_formatDateOnly(assignment.createdDate)} | ${_formatDate(assignment.createdDate)}',
  //                       style: const TextStyle(
  //                         color: AppColors.textGrey,
  //                         fontSize: 10,
  //                       ),
